...
interface Proposal {
    ...
    status: string
    total_amount: number
    proposal_articles: ProposalArticle[]
}
...
const { proposal, clients, articles, suppliers } = defineProps<{
    proposal: Proposal
    clients: Client[]
    articles: Article[]
    suppliers: Supplier[]
...
const form = useForm({
    client_id: (proposal.client_id ?? null) as number | null,
...
onMounted(() => {
    if (proposal.proposal_articles && proposal.proposal_articles.length > 0) {
        articleLines.value = proposal.proposal_articles.map(pa => ({
            article_id: pa.article_id,
            quantity: pa.quantity,
            unit_price: pa.unit_price,
            supplier_id: pa.supplier_id,
            cost_price: pa.cost_price,
        }))
    } else {
        articleLines.value = [{
            article_id: null,
...
function submit() {
    ...
    form.put(`/proposals/${proposal.id}`, {
        onSuccess: () => {
    ...
    <AppLayout>
        ...
        <div class="py-12">
            <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Editar Proposta {{ proposal.number }}</CardTitle>
                    </CardHeader>
                        ...
                        <form class="space-y-6" @submit.prevent="submit">
                            ...
                            <div class="space-y-4">
                                ...
                                <div v-for="(line, index) in articleLines" :key="index"
                                    ...
                                    <div class="col-span-2">
                                        <Input :model-value="line.cost_price ?? ''"
                                            ...
                                            type="number" step="1" min="0" placeholder="Custo (opcional)" />
                                    ...
                                    <div class="col-span-12 text-right text-sm">
                                        ...
                                        <span v-if="line.cost_price" class="ml-4 text-xs text-gray-500">
                                            ...
                                            | Margem: {{ formatPrice((line.quantity * line.unit_price) - (line.quantity * line.cost_price)) }})
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="rounded-md bg-blue-50 p-4">
                                    <div class="flex items-center justify-between">
                                        <span class="text-lg font-medium">TOTAL DA PROPOSTA:</span>
